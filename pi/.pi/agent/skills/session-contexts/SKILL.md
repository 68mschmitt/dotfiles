...
description: Compile the current pi session into durable reference documents in ~/projects/second-brain/session-contexts/{project}/{theme}/. Produces cohesive insight write-ups plus a reasoning-and-intent record extracted from between the lines of the session. Use at the end of a session when the user says "compile this session", "compact this session", "save session context", "distill this session", or wants the session preserved as reference docs.
---

# session-contexts

A session's value evaporates when it ends: the hard-won insights, the why behind decisions, the dead ends that must not be re-walked. This skill compiles the invoking session into a small set of reference documents that a future session (or human) can load cold. It is a distiller, not a logger — write up what mattered, never replay what happened.

Normally invoked at session end; mid-session is fine too (compile what exists so far). If the session contains nothing durable (no real work, decisions, findings, or corrections), say so in one line and stop.
...
3. Pick a lesson-theme slug from the dominant subject of what survived (e.g. `pi-skill-discovery`, never `misc-fixes` or `session-work`). If the session genuinely contains two unrelated lessons, make two theme directories, each complete with its own README and reasoning doc — but prefer one; split only when a single slug would be a junk drawer.
...
5. Write the docs from the templates — [templates/index.md](templates/index.md) → `README.md`, [templates/insight.md](templates/insight.md) → each `insight--{slug}.md`, [templates/reasoning-and-intent.md](templates/reasoning-and-intent.md) → `reasoning-and-intent.md` — following the writing rules below.
