/**
 * OpenAI Codex Usage Extension
 *
 * Publishes your ChatGPT/Codex subscription quota as a pi status, e.g.
 *
 *     73% 2h15m
 *
 * Data source: GET https://chatgpt.com/backend-api/wham/usage
 *   - Uses the existing pi OAuth token from auth.json (`openai-codex`).
 *   - Sends the same `chatgpt-account-id` identity header pi uses for Codex.
...
// Low-remaining coloring (truecolor; matches the powerline palette).
...
async function readCredential(ctx?: ExtensionContext): Promise<Credential | null> {
	// Prefer pi's live model registry so OAuth refreshes performed by pi are honored.
...
export default function (pi: ExtensionAPI) {
	...
	// Refresh after each settle; quota changes as Codex requests are made.
