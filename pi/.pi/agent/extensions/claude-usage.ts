 ...
 * meter that covers you past plan limits) as a pi status: e.g.
 *
 *     credits $68.68/$500 (14%)
 *
 * Data source: GET https://api.anthropic.com/api/oauth/usage
 *   - Uses your existing pi OAuth token (read fresh from auth.json each fetch,
 *     so it tracks pi's own token refresh — no separate OAuth flow needed).
 ...
 * settles (throttled), caches to disk, and backs off aggressively on HTTP 429.
 */
...
// Elevated-usage coloring (truecolor; matches the powerline palette).
const WARN_PCT = 50;
const CRIT_PCT = 80;
